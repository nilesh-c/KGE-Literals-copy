...
def triple2idx(X, entity_textid, empty_vec):
    # Split Text literals
    ...
    for triple in X:
        if idx2entity[triple[0]] in entity_textid:
            train_text_s.append(entity_textid[idx2entity[triple[0]]])
        else:
            train_text_s.append(empty_vec)
...
train_text_s, train_text_o = triple2idx(X_train, entity_textid, empty=vocabulary['<PAD/>'])
...
val_text_s, val_text_o = triple2idx(X_val, entity_textid, empty=vocabulary['<PAD/>'])
...
test_text_s, test_text_o = triple2idx(X_test, entity_textid, empty=vocabulary['<PAD/>'])
test_text_s = csc_matrix(test_text_s)
test_text_o = csc_matrix(test_text_o)
save_npz('data/fb15k-literal/bin/val_text_s.npz',test_text_s)
save_npz('data/fb15k-literal/bin/val_text_o.npz',test_text_o)
