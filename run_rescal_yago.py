...
parser.add_argument('--average_loss', default=False,type=bool,
...
parser.add_argument('--use_subject_lit', default=False, type=bool, metavar='',
                    help='whether to use subject literals (default: False)')
parser.add_argument('--use_object_lit', default=False, type=bool, metavar='',
                    help='whether to use object literals (default: False)')
...
model = RESCAL_literal(n_ent, n_rel, n_lit, k, lam, gpu=args.use_gpu)
# Training params
lr = args.lr
...
checkpoint_path = '{}/RESCAL_literal.bin'.format(checkpoint_dir)
...
for epoch in range(n_epoch):
    ...
    for X_mb in mb_iter:
        ...
        # Training logs
        if it % print_every == 0:
            mrr, hits10 = eval_embeddings_rel(model, X_val, n_rel, 1,
                                              X_lit_s_val, X_lit_o_val)
